# Almost the whole code has been obtained from https://github.com/nar-k/persistent-excitation
...
parser.add_argument(
    '--sd_coef', default=0.0, type=float,
    help='coefficient for spectral decoupling')
...
def train_ce(args,
    ...
    for data1, label1 in pbar:
        ...
        ############# the only change made in the code ##############
        loss = F.cross_entropy(output, label1) + args.sd_coef * (output ** 2).sum()
    ...
    for i in tqdm(range(num_samples_in_dataset)):
        ...
        try:
            adversarial_image = attack(
                img.numpy()[None],
                labels=np.array([true_label_for_dataset]))
...
testa2b = get_adv_disturbance_mag(
    attack=attack,
...
testb2a = get_adv_disturbance_mag(
    attack=attack,
...
traina2b = get_adv_disturbance_mag(
    attack=attack,
...
trainb2a = get_adv_disturbance_mag(
    attack=attack,
...
figure_test_fout = os.path.join('disturbance-fig', figure_test_filename)
...
figure_train_fout = os.path.join('disturbance-fig', figure_train_filename)
